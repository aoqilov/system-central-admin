...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="text-xl font-bold leading-none" style={{ color: "var(--text-default)" }}>
          {value}
        </p>
...
export default function QrCode() {
  ...
  const handleChangeStatus = useCallback(
    async (id: string, status: QrStatus) => { await qr.changeStatus(id, status); },
    [qr.changeStatus],
  );
    ...
    <div className="p-4 tablet:p-6 space-y-5">
      ...
      {qr.activeBatchId && (
        <div className="grid grid-cols-2 desktop:grid-cols-4 gap-3">
          <StatCard icon={IoQrCodeSharp} label="Jami"      value={qr.codes.length} color="#64748b" />
          <StatCard icon={LuQrCode}      label="Faolsiz"   value={noActiveCount}   color="#94a3b8" />
          <StatCard icon={LuQrCode}      label="Faol"      value={activeCount}     color="#22c55e" />
          <StatCard icon={LuQrCode}      label="Ichkarida" value={insideCount}     color="#3b82f6" />
        </div>
      )}
      ...
      {qr.activeBatchId ? (
        ...
        >
          ...
          <p className="text-xs text-center max-w-xs" style={{ color: "var(--text-muted)" }}>
            "Yangi partiya" tugmasini bosib birinchi QR kodlar to'plamini yarating.
          </p>
        </div>
      )}
