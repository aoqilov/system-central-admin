...
interface KassaRow {
  noDiscount: number | null; // null = ma'lumot kiritilmagan
...
// null = ma'lumot kiritilmagan, 0 = kiritilgan lekin qiymat 0
const MOCK_KARTA_SOLD: (number | null)[] = [
  54,
  62,
  91,
  72,
...
// ─── Report table ─────────────────────────────────────────────────────────────
//
// Bu jadval uchun CusTable emas, raw <table> ishlatilgan — sabablari:
//   1. "Итого" qatori — ko'k rang, bold (per-row custom style, CusTable qo'llab-quvvatlamaydi)
//   2. "Sotilgan karta" qatori — sariq rang + body cellda colSpan (CusTable da yo'q)
//   3. Header 2 qatorli, grouped colSpan/rowSpan bor — CusTable children bilan
//      qilish mumkin bo'lardi, lekin yuqoridagi body muammolari tufayli raw <table> tanlandı.
//
// Agar kelajakda CusTable ga getRowStyle prop qo'shilsa, bu jadval ham
// CusTable orqali yozilishi mumkin.
...
function ReportTable({
  ...
  // "Тип оплаты" kolonnasini kichik ekranlarda chap tomonda fix qilish
        ...
        >
          <thead>
            <tr>
              ...
              <th
                style={{ ...thStyle, textAlign: "center", borderRight: "none" }}
              >
                Итого
              </th>
            </tr>
          ...
          <tbody>
            ...
            {rows.map((row) => {
              ...
              return (
                <tr key={row.type} style={{ background: "var(--bg-second)" }}>
                  ...
                  <td
                    style={{ ...tdNum, fontWeight: 600, borderRight: "none" }}
                  >
                    {fmtNum(rowTotal)}
                  </td>
                </tr>
            ...
            <tr style={totalRow}>
              ...
              {KASSAS.map((_, ki) => (
                <td
                  key={ki}
                  style={{ ...tdNum, ...totalRow, color: "#60a5fa" }}
                >
...
export default function RoleKassaMainExport() {
  ...
  // Ma'lumot kiritilgan kassalar soni (kamida 1 to'lov turi null emas)
  const activeKassaCount = KASSAS.filter((_, ki) =>
    MOCK_ROWS.some((r) => r.kassas[ki]?.noDiscount !== null),
  ).length;
  ...
  async function exportToExcel() {
    ...
    a.href     = url;
    a.download = `Z-otchet_${PARK_NAME}_${dateLabel}.xlsx`;
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          <CusSegment
            ...
            value={dateMode}
            onValueChange={(v) => {
              setDateMode(v as "kunlik" | "oraliq");
            }}
          />

          {/* Kunlik: < date > navigator */}
          {dateMode === "kunlik" ? (
            <div
            ...
            <div className="flex items-center gap-2">
              <div style={{ width: 160 }}>
                <CusCalendar
                  ...
                  onValueChange={({ value }) => {
                    if (value[0]) {
                      setDateFrom(calDateToStr(value[0]));
                    }
                  }}
              ...
              <div style={{ width: 160 }}>
                <CusCalendar
                  ...
                  onValueChange={({ value }) => {
                    if (value[0]) {
                      setDateTo(calDateToStr(value[0]));
                    }
                  }}
      ...
      <CusDialog
        ...
        footer={
          <>
            ...
            <CusButton colorPalette="blue" onClick={handleSend}>
      ...
      >
        <div className="flex flex-col gap-0">
          {[
            ...
            {
              label: "Продано карт",
              value: `${MOCK_KARTA_SOLD.reduce<number>((s, n) => s + (n ?? 0), 0)} шт.`,
            },
            {
              label: "Общая выручка",
