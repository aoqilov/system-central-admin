...
const STATUS_CONFIG: Record<RowStatus, { label: string; colorPalette: "gray" | "green" | "red" }> = {
...
export default function RoleOperatorMainIncoming() {
  ...
  function toggleSmena(id: number) {
    setRows((prev) => prev.map((r) => r.id === id ? { ...r, smenaClose: !r.smenaClose } : r));
  }
  ...
  function handleExport() {
    ...
    const td = (text: string | number, align = "center", color = "#1e293b", bold = false) =>
    ...
    const statusColor: Record<RowStatus, string> = {
      pending: "#6b7280", confirmed: "#16a34a", rejected: "#dc2626",
    };
...
<thead><tr>
  ...
  ${th("Haq to'langan","right")}${th("Jami summa","right")}${th("Holat")}
</tr></thead>
  ...
  return (
    ...
    <div className="p-4 desktop:p-6 space-y-4 pb-8">
      ...
      >
        ...
        <div className="flex-1 flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            ...
            <p className="font-semibold text-sm" style={{ color: sentAt ? "#22c55e" : "var(--text-default)" }}>
              {sentAt ? "Buxgalterga jo'natildi" : "Buxgalterga jo'natish"}
            </p>
            {sentAt && (
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1">
            {[
              { icon: LuFerrisWheel, label: `${rows.length} ta attraksiya`    },
              { icon: LuPlay,        label: `${totalRounds} round`             },
              { icon: LuUsers,       label: `${totalCards.jami} ta karta`      },
        ...
        {/* Right: button */}
        {sentAt ? (
          <CusBadge colorPalette="green" dot size="md">Юборилди</CusBadge>
        ) : (
          <CusButton
            colorPalette="blue"
            size="sm"
            onClick={() => { setNote(""); setSendDialog(true); }}
          >
            <LuSend size={14} /> Buxgalterga jo'natish
          </CusButton>
        )}
      </div>
        ...
        <div className="overflow-x-auto">
          <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 1280 }}>
            <thead>
              {/* Row 1 */}
              <tr>
                <th rowSpan={2} style={{ ...thBase, textAlign: "center", paddingLeft: 12, paddingRight: 12, width: 40 }}>
                  #
                </th>
                <th rowSpan={2} style={{ ...thBase, textAlign: "left", paddingLeft: 16, minWidth: 180 }}>
                  Привлечение
                </th>
                ...
                <th rowSpan={2} style={{ ...thBase, textAlign: "center", borderRight: "none", minWidth: 280 }}>
                  Amal
                </th>
              </tr>
              ...
              <tr>
                {TABLE_COLS.map((c) => (
                  <th
                    key={c.key}
                    style={{ ...thBase, textAlign: "right", color: c.color, paddingTop: 3 }}
                  >
                    {c.label}
                  </th>
                ))}
            ...
            <tbody>
              {rows.map((row, ri) => (
                <tr key={row.id}>
                  ...
                  {/* # */}
                  <td style={{ ...tdBase, textAlign: "center", color: "var(--text-dim)", fontSize: 12 }}>
                    {ri + 1}
                  </td>
                  ...
                  <td style={{ ...tdBase, borderRight: "none" }}>
                    {row.status === "pending" ? (
                      ...
                      <CusBadge colorPalette={STATUS_CONFIG[row.status].colorPalette} dot>
            ...
            <tfoot>
              <tr style={{ borderTop: "2px solid var(--border-default)", background: "var(--bg-hover)" }}>
                <td
                  colSpan={3}
                  style={{ ...tdBase, paddingLeft: 16, fontWeight: 700, color: "var(--text-muted)", fontSize: 12 }}
                >
                  Итого
                </td>
                {TABLE_COLS.map((c) => {
                ...
                <td style={{ ...tdBase, textAlign: "right", fontWeight: 700, fontSize: 13 }}>
                  {fmt(totalPaid)}
                </td>
                <td style={{ ...tdBase, textAlign: "right", fontWeight: 700, fontSize: 13, color: "#22c55e" }}>
                  {fmt(totalRevenue)}
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div>
          {[
            { label: "Sana",           value: tab === "daily" ? selectedDay.format("DD.MM.YYYY") : `${dateFrom[0] ?? "—"} — ${dateTo[0] ?? "—"}` },
            { label: "Attraksiyalar",  value: `${rows.length} ta`               },
            { label: "Jami round",     value: `${totalRounds} ta`               },
            { label: "Jami karta",     value: `${totalCards.jami} ta`           },
        ...
        <CusTextArea
          ...
          placeholder="Buxgalterga qo'shimcha ma'lumot..."
          autoresize
          maxH="120px"
          value={note}
