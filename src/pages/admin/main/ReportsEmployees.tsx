...
function genEfficiency(e: Employee, di: number): number {
  ...
  return Math.min(
    100,
    Math.round(base * (0.9 + _s(e.id * 23 + di * 13) * 0.2)),
  );
}
...
const ReportsEmployees = () => {
  ...
  const dailyHoursData = useMemo(
    () =>
      dates.map((date, di) => {
        const d = dayjs(date);
        const totalMins = employees.reduce(
          (s, e) => s + genWorkedMinutes(e, di),
          0,
        );
        return {
    ...
    () =>
      ROLES.map((role) => {
        ...
        const totalMins = roleEmps.reduce(
          (s, e) =>
            s + dates.reduce((ds, _, di) => ds + genWorkedMinutes(e, di), 0),
          0,
        );
        return {
  ...
  const summary = useMemo<EmployeeSummary[]>(
    () =>
      employees.map((e) => {
        ...
        const salary = e.salary ? Math.round(e.salary * (daysWorked / 26)) : 0;
        return {
          id: e.id,
  ...
  // Aggregates
  const totalManHours = Math.round(
    summary.reduce((s, e) => s + e.totalMinutes, 0) / 60,
  );
  ...
  const avgAttendance = Math.round(
    employees
      ...
      (employees.filter((e) => (e.statsUser?.core.attendanceRate ?? 0) > 0)
        .length || 1),
  );
