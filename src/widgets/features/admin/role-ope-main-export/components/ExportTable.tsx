        ...
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 1000 }}>
          <thead>
            <tr>
              ...
              <th rowSpan={2} style={{ ...thBase, textAlign: "right", borderRight: "none" }}>Итого</th>
            </tr>
            <tr>
              {TABLE_COLS.map((c) => (
          ...
          <tfoot>
            <tr style={{ borderTop: "2px solid var(--border-default)", background: "var(--bg-hover)" }}>
              ...
              {TABLE_COLS.map((c) => {
                ...
                const isDefault = c.color === "var(--text-default)";
                return (
                  <td key={c.key} style={{ ...tdBase, textAlign: "right", fontWeight: 700, fontSize: 13, color: isDefault ? "var(--text-default)" : c.color }}>
                    {total > 0 ? total : "—"}
                  </td>
                );
              })}
              <td style={{ ...tdBase, textAlign: "right", fontWeight: 700, fontSize: 13 }}>{fmt(totalPaid)}</td>
