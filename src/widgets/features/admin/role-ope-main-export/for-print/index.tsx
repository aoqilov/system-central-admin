...
export function exportToExcel({ rows, filename }: ExportParams) {
  ...
  const footerRow = [
    ...
    rows.reduce((s, r) => s + r.cards.mehmon, 0),
    rows.reduce((s, r) => s + r.cards.parkXodim, 0),
    totalPaid,
    totalRevenue,
  ];
  ...
  ws["!merges"] = [
    ...
    { s: { r: 0, c: 3 }, e: { r: 0, c: 8 } }, // Тип карты
    { s: { r: 0, c: 9 }, e: { r: 1, c: 9 } }, // Оплачено
    { s: { r: 0, c: 10 }, e: { r: 1, c: 10 } }, // Итого
