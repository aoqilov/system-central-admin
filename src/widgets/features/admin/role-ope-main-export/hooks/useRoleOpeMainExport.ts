...
import { getAccountingReports } from "../api/apiOpeMainExport";
import { exportToExcel } from "../for-print";
import { type AttractionRow, type CardCounts } from "../types";
...
export function useRoleOpeMainExport() {
  ...
  const params = useMemo(() => {
    if (tab === "daily") {
      return { date: selectedDay.format("YYYY-MM-DD") };
    }
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn:  () => getAccountingReports(params!),
    enabled:  !!params,
  });
  ...
  const rows: AttractionRow[] = useMemo(() => {
    ...
    return data.attractions.map(({ attraction, zreport }) => ({
      ...
      cards: {
        ...
        mehmon:    zreport.total_guest,
        parkXodim: zreport.total_park_staff,
      },
