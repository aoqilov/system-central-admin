...
export function AssignOperatorModal({ open, attractionId, onClose }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      assignAttractionOperator(attractionId, { operator: selectedEmp!, type: "main" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["attraction-detail", attractionId] });
      handleClose();
    },
  });
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchEmployees({ limit: 100, roles: 3 }),
    enabled: open,
  });
