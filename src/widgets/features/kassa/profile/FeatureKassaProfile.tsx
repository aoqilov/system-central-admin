...
const DEMO_KASSA =
  kassaList.find((k) => k.cashierId === DEMO_CASHIER?.id) ?? kassaList[0];
    ...
    <div className="p-4 tablet:p-6 space-y-4 pb-6">
      ...
      >
        ...
        <div className="px-4 pb-4">
          ...
          >
            {emp.phone && (
              <InfoRow icon={LuPhone} label="Telefon" value={emp.phone} />
            )}
            {emp.telegram_username && (
              <InfoRow
                icon={LuSend}
                label="Telegram"
                value={emp.telegram_username}
              />
            )}
            <InfoRow
              icon={LuBriefcase}
              label="Kassa"
              value={`${kassa.name} — ${kassa.location}`}
            />
            <InfoRow
              icon={LuCalendar}
              label="Ishga kirgan"
