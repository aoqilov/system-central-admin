...
export function CloseXReportDialog({ open, xreport, index, onClose, onConfirm }: Props) {
  ...
  const rows = [
    ...
    { label: "Jami summa", value: `${fmt(xreport.total_amount)} сум` },
  ];
  ...
  return (
    <CusDialog
      open={open}
      onClose={onClose}
      title={`X-otchet #${index} ni yopish`}
      description="Yopilgandan so'ng qayta ochib bo'lmaydi."
      size="sm"
      footer={
        <>
