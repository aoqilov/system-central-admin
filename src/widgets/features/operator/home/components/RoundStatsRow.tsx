    ...
    >
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            ...
            <span className="text-xs" style={{ color: "var(--text-muted)" }}>
              Yopildi:{" "}
              <span style={{ color: "var(--text-default)" }}>
                {activeXreport?.closed_at ? toTime(activeXreport.closed_at) : "—"}
              </span>
            </span>
          </div>
        </div>
      ...
      <div className="overflow-x-auto px-4 py-4">
        <div className="flex gap-3">
          <StatCard icon={LuPlay}     label="Round"      value={String(roundCount)} color="#3b82f6" />
          <StatCard icon={LuUsers}    label="Jami"       value={String(people)}     color="#22c55e" />
          <StatCard icon={LuWifiOff}  label="Offline"    value={String(offline)}    color="#64748b" />
          <StatCard icon={LuWifi}     label="Online"     value={String(online)}     color="#06b6d4" />
          <StatCard icon={LuStar}     label="VIP"        value={String(vip)}        color="#eab308" />
          <StatCard icon={LuUserPlus} label="Mehmon"     value={String(guest)}      color="#8b5cf6" />
          <StatCard icon={LuShield}   label="Park xodim" value={String(parkStaff)}  color="#22c55e" />
          <StatCard icon={LuBanknote} label="Jami summa" value={fmt(total)} sub="сум" color="#3b82f6" />
        </div>
      </div>
    </div>
  );
